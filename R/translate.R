...
replace_image <- function(x, image_size_default = 600) {
  ...
  for (loc in rev(split(locs, row(locs)))) {
    ...
    stringi::stri_sub(x, loc[1], loc[2]) <- glue(
      '<ac:image {hw_params}><ri:attachment ri:filename="{basename(src)}" /></ac:image><span class="jira-issue" style="display:block;max-width:600px"><em>{title}</em></span>'
    )
  }
  x
}
